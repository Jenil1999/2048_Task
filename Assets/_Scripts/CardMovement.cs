...
public class CardMovement : MonoBehaviour
{
    ...
    private void OnMouseUp()
    {
        if (_isCollide && enabled)
        {
            indexNumber = BlockManager.Instance.AllCorrectPositions.IndexOf(_pos);
                ...
                {
                    SufflingCards.Instance.cardCount++;
                    if (_isMoving)
                    {
                       ...
                       transform.DOScale(new Vector3(1, 1, 1), .15f).OnComplete(() =>
                       {
                           ...
                           if (indexNumber != -1)
                           {
                               ...
                               if (transform.CompareTag("Card"))
                               {
                                   ...
                                   if (indexNumber == 0)
                                   {
                                   //if (BlockManager.Instance.Block1.Count <= 7)
                                   //{
    ...
    {
        if(collision.gameObject.CompareTag("Discard") && OnDiscard.instance.discardedCardCount < 2)
        {
            OnDiscardCard();
        }
    ...
    private void OnDiscardCard()
    {
        ...
        if(OnDiscard.instance.discardedCardCount == 1)
        {
            OnDiscard.instance.upperImage.color = Color.red;
        }
        else if(OnDiscard.instance.discardedCardCount == 2)
        {
            OnDiscard.instance.downImage.color = Color.blue;
        }
